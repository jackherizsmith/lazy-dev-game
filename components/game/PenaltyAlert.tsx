...
export function PenaltyAlert({ outcome, penalty, onClose }: PenaltyAlertProps) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="rounded-lg border-2 border-red-500 bg-red-50 dark:bg-red-950 p-6 shadow-xl">
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0">
              <span className="text-4xl">⚠️</span>
            </div>
            <div className="flex-1">
              <h3 className="mb-2 text-lg font-bold text-red-900 dark:text-red-100">
                It Backfired! ({penalty} points)
              </h3>
              <p className="text-red-800 dark:text-red-200">{outcome}</p>
            </div>
            <button
              onClick={onClose}
